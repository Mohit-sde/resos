...
import { ChevronLeft, Package, Clock, Phone, AlertCircle } from "lucide-react";
...
export default function RestaurantOrderDetailPage() {
  ...
  // Fetch order
  useEffect(() => {
    if (!restaurant || !currentUser?.uid || !orderId) return;

    const fetchOrder = async () => {
      try {
        const orderData = await getOrderById(restaurant.restaurant_id, orderId);
        ...
        // Check authorization
        if (orderData.customer_id !== currentUser.uid) {
    ...
    fetchOrder();
  }, [restaurant, currentUser?.uid, orderId]);
                ...
                {idx < statusFlow.length - 1 && (
                  <div
                    className={`flex-1 h-1 mx-1 ${
                      idx < currentStatusIndex ? "bg-gray-200" : "bg-gray-200"
                    }`}
                    style={
                      idx < currentStatusIndex
                        ? { backgroundColor: brandColor }
